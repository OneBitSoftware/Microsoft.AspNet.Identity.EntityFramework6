namespace Microsoft.AspNet.Identity.EntityFramework6
{
        ...
        where TKey : IEquatable<TKey>
    {
        ...
        public async virtual Task AddToRoleAsync(TUser user, string roleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            //casting ID tostring here due to EF6 not supporting generic types for DbSet
            var ur = new IdentityUserRole { UserId = ConvertIdToString(user.Id), RoleId = ConvertIdToString(roleEntity.Id) };
            UserRoles.Add(ur);
        }
        ...
        public async virtual Task RemoveFromRoleAsync(TUser user, string roleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (string.IsNullOrWhiteSpace(roleName))
            {
                throw new ArgumentException("Value cannot be null of empty: {0}", nameof(roleName));
            }
            var roleEntity = await Roles.SingleOrDefaultAsync(r => r.Name.ToUpper() == roleName.ToUpper(), cancellationToken);
            if (roleEntity != null)
            {
                var roleEntityId = ConvertIdToString(roleEntity.Id);
                var userId = ConvertIdToString(user.Id);
                var userRole = await UserRoles.FirstOrDefaultAsync(r => roleEntityId.Equals(r.RoleId)
                && r.UserId.Equals(userId), cancellationToken);
                if (userRole != null)
                {
        ...
        public virtual async Task<IList<string>> GetRolesAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            //Added castring ToString() for EF6 support
            var userId = ConvertIdToString(user.Id);
            var query = from userRole in UserRoles
                        join role in Roles on userRole.RoleId equals role.Id.ToString()
        ...
        public virtual async Task<bool> IsInRoleAsync(TUser user, string roleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (role != null)
            {
                ...
                //casting ToString here due to EF6 not supporting generic DbSets
                return await UserRoles.AnyAsync(ur => ur.RoleId.Equals(roleId.ToString()) && ur.UserId.Equals(userId.ToString()));
            }
            return false;
        }
        ...
        public async virtual Task<IList<Claim>> GetClaimsAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var userId = ConvertIdToString(user.Id);
            var result = await UserClaims.Where(uc => uc.UserId.Equals(userId))
        ...
        public virtual Task AddClaimsAsync(TUser user, IEnumerable<Claim> claims, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                //casting ID tostring here due to EF6 not supporting generic types for DbSet
                UserClaims.Add(new IdentityUserClaim
                {
        ...
        public async virtual Task ReplaceClaimAsync(TUser user, Claim claim, Claim newClaim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var userId = ConvertIdToString(user.Id);
            var matchedClaims = await UserClaims.Where(uc => uc.UserId.Equals(userId)
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var userId = ConvertIdToString(user.Id);
            var entry = await UserLogins.SingleOrDefaultAsync(l => l.UserId.Equals(userId)
        ...
        public async virtual Task<IList<UserLoginInfo>> GetLoginsAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var userId = ConvertIdToString(user.Id);
            var userLogins = await UserLogins.Where(l => l.UserId.Equals(userId)).ToListAsync(cancellationToken); ;
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (userLogin != null)
            {
                //casted ToString to support EF6
                return await Users.FirstOrDefaultAsync(u => u.Id.ToString().Equals(userLogin.UserId), cancellationToken);
            }
            return null;
        }
        ...
        public async virtual Task<IList<TUser>> GetUsersForClaimAsync(Claim claim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            //casted Id ToString() due to the EF6 generic type support in DbSet
            var query = from userclaims in UserClaims
                        join user in Users on userclaims.UserId equals user.Id.ToString()
